...
static func from_body_direct_state(state: PhysicsDirectBodyState2D) -> PhysicsBodySnapshot2D:
	...
	for contact_idx in range(state.get_contact_count()):
		res.contact_collider.push_back(state.get_contact_collider(contact_idx))
		res.contact_collider_id.push_back(state.get_contact_collider_id(contact_idx))
		res.contact_collider_object.push_back(state.get_contact_collider_object(contact_idx))
		res.contact_collider_position.push_back(state.get_contact_collider_position(contact_idx))
		res.contact_collider_shape.push_back(state.get_contact_collider_shape(contact_idx))
		res.contact_collider_velocity_at_position.push_back(state.get_contact_collider_velocity_at_position(contact_idx))
		res.contact_local_normal.push_back(state.get_contact_local_normal(contact_idx))
		res.contact_local_position.push_back(state.get_contact_local_position(contact_idx))
		res.contact_local_shape.push_back(state.get_contact_local_shape(contact_idx))
